...
import fs from 'fs'
import type { SVGFolderInterface, SVGToFile} from './types/plugin.interface'
        ...
        configureServer(server: ViteDevServer) {
            ...
            server.watcher.on('add', (filePath) => {
                if (dirname(filePath) === watchDir && !basename(filePath)?.includes(nameOfTheOutputFile) && extname(filePath) === '.svg') {
                    ...
                    setTimeout(() => {
                        try {
                            if(Number.parseInt(convertType ?? '') === 0)
                                convertSVGToFile({ filePath, watchDir, nameOfTheOutputFile, translation, relativePath, dummy })
                            else
                                convertSVGToFonts()
                        } catch (err) {
...
function convertSVGToFile({ filePath, watchDir, nameOfTheOutputFile, translation, relativePath, dummy }: SVGToFile) {
    const   
